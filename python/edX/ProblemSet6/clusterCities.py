...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        # TO DO
        mc=c1.points + c2.points
        pType=c1.pointType
        self.members.append(Cluster(mc, pType))
        try:
    ...
    def findClosest(self, linkage):
        ...
        # TO DO
        minClust=None
        minDist = 10000000000000.00
        for c1 in self.members:
            for c2 in self.members:
                if c1 != c2:
                    dist = float(linkage(c1,c2))
                    if minDist > float(dist):
                        minDist = float(dist)
                        minClust = tuple([c1, c2])
        return minClust
    ...
    def mergeOne(self, linkage):
        ...
        # TO DO
        merClus = self.findClosest(linkage)
        self.mergeClusters(merClus[0], merClus[1])
        return merClus
