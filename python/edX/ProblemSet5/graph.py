...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        return Edge.__str__(self) + ' (' + str(self.totalDist) + ', ' + str(self.outdoorDist) + ')'
...
class WeightedDigraph(Digraph):
    ...
    def addEdge(self, edge):
        ...
        #distance = (totDist, outDist)
        if not(src in self.nodes and dest in self.nodes):
            raise ValueError('Node not in graph')
        self.edges[src].append([dest,tuple((float(totDist), float(outDist)))])
        #self.edges[src].append([dest, tuple(distance)])
    def childrenOf(self, node):
