...
averageTrainYear = (parsedTrainData
                    .map(lambda x : x.label).reduce(lambda x, y: x + y))/ parsedTrainData.count()
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return math.sqrt(labelsAndPreds.map(lambda (x, y): squaredError(x, y)).reduce(lambda x, y: x+y) / labelsAndPreds.count())
...
def twoWayInteractions(lp):
    ...
    """
    nFeatures = list(itertools.product(lp.features,repeat=2))
    nList = [x*y for (x,y) in nFeatures]
    finalfeatures=np.hstack((lp.features,nList))
