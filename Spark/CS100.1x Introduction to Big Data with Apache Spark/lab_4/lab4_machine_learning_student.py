...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    return (IDandRatingsTuple[0], (numRating, sum(IDandRatingsTuple[1]) / float(numRating)))
...
movieIDsWithRatingsRDD = (ratingsRDD
                          .map(lambda (user, mid, rate) : (mid, rate))).groupByKey()
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              .join(movieIDsWithAvgRatingsRDD).map(lambda (x, (y, z)) : (z[1], y, z[0])))
...
def computeError(predictedRDD, actualRDD):
    ...
    totalError = squaredErrorsRDD.map(lambda (x, y): (y[0] - y[1])**2).reduce(lambda x, y: x+y)
...
myUnratedMoviesRDD = (moviesRDD
                      .leftOuterJoin(myRatingsRDD.map(lambda (x, y,z) : (y, x))).filter(lambda (x,(y, z)): z==None)).map(lambda (x, y): (myUserID, x))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD).map(lambda (x, ((p, q), r)): (p, r, q) ))
