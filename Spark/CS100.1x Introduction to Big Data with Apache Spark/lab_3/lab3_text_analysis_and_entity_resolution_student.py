...
def simpleTokenize(string):
    ...
    """
    return filter(lambda x: x!= '', re.compile(split_regex).split(string.lower()))
...
def tokenize(string):
    ...
    return filter(lambda x: x!= '' and x not in stopwords, re.compile(split_regex).split(string.lower()))
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda (id_1, val): (id_1, tokenize(val)))
googleRecToToken = googleSmall.map(lambda (id_1, val): (id_1, tokenize(val)))
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda (k, v) : len(v)).reduce(lambda x, y: x + y)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return [max(vendorRDD.collect(), key=lambda (x, y): len(y))]
...
def tfidf(tokens, idfs):
    ...
    tfs = tf(tokens)
    tfIdfDict = dict(tfs)
    for key, val in tfIdfDict.items():
        if key in idfs:
            tfIdfDict[key] = tfIdfDict[key] * idfs[key]
    return tfIdfDict
...
avgSimDups = trueDupsRDD.map(lambda (x, y) : y[0]).reduce(lambda x, y : x + y) /float(trueDupsCount)

nonDupsRDD = (sims
              .leftOuterJoin(goldStandard).filter(lambda (x, y) : y[1]==None))
avgSimNon = nonDupsRDD.map(lambda (x, y) : y[0]).reduce(lambda x, y : x + y) /float(nonDupsRDD.count())
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda (id_1, val): (id_1, tokenize(val)))
googleFullRecToToken = google.map(lambda (id_1, val): (id_1, tokenize(val)))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = dict(amazonWeightsRDD.map(lambda (x, y) : (x, norm(y))).collect())
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = dict(googleWeightsRDD.map(lambda (x, y) : (x, norm(y))).collect())
...
def invert(record):
    ...
    """
    pairs = [(recval, record[0]) for recval in record[1].keys()]
    return (pairs)
...
commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD).map(swap).map(lambda (x, y) : (x,[y])).reduceByKey(lambda x, y : x + y)
                .cache())
