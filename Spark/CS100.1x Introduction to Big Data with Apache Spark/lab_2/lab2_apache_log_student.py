...
dayHostCount = dayGroupedHosts.reduceByKey(lambda x, y : x + y).sortByKey()
...
groupedByDay = dayAndHostTuple.map(lambda (x, y) : (x, 1)).reduceByKey(lambda x, y : x + y).join(dailyHosts).map(lambda (x,(y, z)) : (x, y/z))
